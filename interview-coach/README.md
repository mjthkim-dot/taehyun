...
| 🔴 라이브 모드 | `/live.html` | 실전 화상면접(Zoom/Meet) 창 옆에 띄우는 실시간 코파일럿 (Smooth AI UI/UX 벤치마킹) |

### 라이브 모드 화면 구성 (2패널, 라이트 테마)

```
┌─ 💬 실시간 자막 ──────────────┬─ ✨ AI 답변 제안 ────────────────┐
│ 면접관 영어 발화 (실시간)       │ 질문 감지 시 자동 생성:           │
│  └ 한국어 번역 (스트리밍)       │  💡 전략 한 줄 (한국어)           │
│ [질문 감지] 하이라이트 카드      │  ① 간결한 답변 EN + 한국어 번역   │
│                              │  ② 강한 답변(숫자 포함) EN + 번역  │
│ 요약 배너 (요청 시)            │  각 답변 🔊 듣기                  │
├──────────────────────────────┤ 🇰🇷 한국어 입력 → 영어 변환        │
└──────────────────────────────┴──────────────────────────────────┘
...
- **영어 → 한국어 실시간 번역**: 면접관 발화가 자막으로 뜨고 바로 아래 한국어 번역이 토큰 단위로 흘러나온다. 답변 제안 요청이 있으면 그것이 항상 우선 처리된다.
- **영어 답변셋**: 질문마다 서로 다른 각도의 답변 2개(간결형 / 숫자 포함 강조형)가 각각 한국어 번역과 함께 스트리밍된다. 실시간성이 핵심이므로 `GROQ_API_KEY` 사용을 강력 권장 (1~2초 내 표시).
