 ...
 *   ④ 완료는 관찰 기반 — 발화 수가 목표를 넘으면 '말하기' 블록이 자동 확인된다
...
check('시작 전엔 서약 카드가 홈 첫 화면에 뜬다', await page.evaluate(() => document.body.innerText.includes('12주 코스')));
...
/* ④ 관찰 기반 완료 — 발화 25문장을 채우면 말하기 블록이 자동 확인 */
await page.evaluate(() => {
...
check('발화 목표를 넘기면 말하기 블록이 자동 확인된다', await page.evaluate(() => {
  const li = [...document.querySelectorAll('.pg-block')].find((e) => (e.querySelector('.pg-block-title')?.textContent || '').startsWith('말하기'));
...
await page.waitForFunction(() => document.body.innerText.includes('오늘 레슨 완료'), null, { timeout: 8000 });
...
check('다시 열어도 완료 상태가 유지된다', await page.evaluate(() => document.body.innerText.includes('오늘 레슨 완료')));
